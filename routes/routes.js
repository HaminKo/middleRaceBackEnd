...
router.post('/games/updatePlayerTargets/:id', function(req, res, next) {
  Game.findById(req.params.id, function(err, game) {
    if (err) {
      res.status(500).json({
        success: false,
      ...
      var updateUserArray = game.users.slice(0)
      var newPlayerTargetsArray = req.body.playerPositionsArray.slice(0)
      for (var i = 0; i < newPlayerTargetsArray.length; i++) {
        updateUserArray[i].previousTarget = updateUserArray[i].currentTarget
        updateUserArray[i].currentTarget = newPlayerTargetsArray[i]
      }
      game.update({
        users: updateUserArray
       }, function(err, game) {
